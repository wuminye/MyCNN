...
for i = num_sublayer:-1: 2
     ...
     for j = 1 : num_subnet
        %  res{i}{j} = cell(length(model.sublayer{i}.subnet{j}.model.Layer),1);
          ...
          %===== GPU =========
              theta.t = gpuArray(theta.t);
              theta.w = gpuArray(theta.w);
          %===================
          if i ~= num_sublayer              
             %与下一sublayer 的subnet的最多连接个数； 需要粘合剂
             n_con = length(model.sublayer{i+1}.subnet);  
             
             theta.t = gpuArray(zeros(size(data{i}{j}{end})));
             ...
             %---------计算梯度-------------------------
                ...
                if strcmp(cur,'Conv') 
                    ...
                    theta.w = gpuArray(zeros(size(model_layer.w)));
                    theta.b = gpuArray(zeros(size(model_layer.b)));
                   ...
                   theta.t = theta.t.*deActiveFunction(data{i}{j}{end});
                    ...
                    for q = 1 : size( theta.w,4)
                       for p = 1 : size(theta.w,3)
                           ...
                           tem = gpuArray(zeros( model.sublayer{i}.subnet{j}.model.Layer{end-1}.out(1:2)));
          ...
          res{i}{j} = gpu_cnnSubNetGrad(model.sublayer{i}.subnet{j}.model,...
                                    data{i}{j},theta);
