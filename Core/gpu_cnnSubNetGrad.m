...
%=====================误差矩阵计算=================================
    ...
    if strcmp(nex,'Conv')
        res{i}.t = gpuArray(zeros(model.Layer{i}.out));
        ...
        for p = 1:size(res{i}.t,3)
            for q = 1:size(res{i+1}.t,3)
               ...
               res{i}.t(:,:,p) = res{i}.t(:,:,p) + ...
                        conv2(res{i+1}.t(:,:,q),gpuArray(model.Layer{i+1}.w(:,:,p,q)),'full');
            end
        end  
    end
     ...
     if strcmp(nex,'Convs')
        ...
        for p = 1:size(res{i}.t,3)
            for q = 1:size(res{i+1}.t,3)
               ...
               tem = gpuArray(zeros(model.Layer{i}.out(1:2)));
               tem(1:model.Layer{i+1}.stride:end,1:model.Layer{i+1}.stride:end)...
                   = res{i+1}.t(:,:,q);
               
               res{i}.t(:,:,p) = res{i}.t(:,:,p) + ...
                        conv2(tem,gpuArray(model.Layer{i+1}.w(:,:,p,q)),'same');
            end
        end  
    end
...
%================梯度计算================================================    
    ...
    if strcmp(cur,'Conv') 
        ...
        res{i}.w = gpuArray(zeros(size(model.Layer{i}.w)));
        res{i}.b = gpuArray(zeros(size(model.Layer{i}.b)));
    ...
    if strcmp(cur,'Convs') 
        ...
        for q = 1 : size(res{i}.w,4)
           for p = 1 : size(res{i}.w,3)
               ...
               tem = gpuArray(zeros(model.Layer{i-1}.out(1:2)));
               ...
               nf = gpuArray(zeros(size(tem,1)+hk(1)*2,...
                          size(tem,2)+hk(2)*2));
