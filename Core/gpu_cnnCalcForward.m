function [ res , inFeatureMap] = cnnCalcForward( model ,data ,state )
%对于整合网络的计算
%data 为featuremap格式
    ...
    for j = 1:num_subnet 
        ...
        for k = 1: n_con
           ...
           % inFeatureMap = mergeFeatureMap(inFeatureMap,res{i-1}{k}{end});
            ...
            tem(:,:,size(inFeatureMap,3)+1:end)=gather(res{i-1}{k}{end});
            inFeatureMap = tem;
        ...
        res{i}{j} = ...
            gpu_cnnCalcSubnet( model.sublayer{i}.subnet{j}.model ,gpuArray(inFeatureMap) );
